import { Package, Upload, X, Shield, Mail, MessageSquare, Layers, Bell, AlertTriangle, BarChart3, CheckCircle, ChevronDown, FileText, Database, Trash2, RefreshCw, Plus } from "lucide-react";
  ...
  {hasPermission('settings') && <div className="card" style={{padding:'24px 28px',marginBottom:16}}>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:10}}>
      {Object.entries(waMessageTemplates).map(([key, tmpl]) => {
        const icons = { orderApproval: Shield, bulkApproval: Layers, backOrder: AlertTriangle, deliveryArrived: null, stockAlert: Bell, monthlyUpdate: BarChart3, partArrival: CheckCircle };
    ...
    <div style={{marginTop:12,display:'flex',gap:8}}>
      ...
      <button className="bs" style={{fontSize:12}} onClick={()=>{setWaMessageTemplates({
        orderApproval: { label: 'Order Approval Request', message: '*\u{1F4CB} Order Approval Request*\n\nRequested By: {orderBy}\nDate: {date}\nOrders: {orderCount}\nTotal Qty: {totalQty} units\nTotal: *S${totalCost}*\n\n{orderTable}\n\n_Reply *APPROVE* or *REJECT*_\n_Miltenyi Inventory Hub SG_' },
        bulkApproval: { label: 'Bulk Order Approval', message: '*\u{1F4CB} Bulk Order Approval Request*\n\nRequested By: {orderBy}\nDate: {date}\nBatches: {batchCount}\nItems: {itemCount}\nTotal Qty: {totalQty} units\nTotal: *S${totalCost}*\n\n{orderTable}\n\n_Reply *APPROVE* or *REJECT*_\n_Miltenyi Inventory Hub SG_' },
