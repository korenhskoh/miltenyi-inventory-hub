      ...
      <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
            ...
            <div>
              <h3 style={{ fontSize: 15, fontWeight: 600 }}>Baileys WhiskeySockets</h3>
              <p style={{ fontSize: 11, color: '#94A3B8' }}>WhatsApp Web Multi-Device API</p>
            </div>
          </div>
          ...
          <div
            ...
            style={{
              padding: '20px 24px',
              marginBottom: 16,
              border: waConnected ? '2px solid #25D366' : '2px solid #E2E8F0',
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  <div style={{ fontWeight: 600, fontSize: 14, color: waConnected ? '#0B7A3E' : '#64748B' }}>
                    {waConnected ? 'Connected' : 'Disconnected'}
                  </div>
                  <div style={{ fontSize: 11, color: '#94A3B8' }}>
                    {waConnected ? 'Session active via Baileys Multi-Device' : 'Scan QR code to connect'}
                  </div>
                </div>
              </div>
              <div
                style={{
                  ...
                  animation: waConnected ? 'pulse 2s infinite' : 'none',
                }}
              />
            </div>
            ...
            {waConnected && waSessionInfo && (
              <div style={{ padding: 12, borderRadius: 8, background: '#F0FDF4', marginBottom: 16, fontSize: 12 }}>
                <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  <div>
                    Phone: <strong className="mono">{waSessionInfo.phone}</strong>
                  </div>
                  <div>
                    Name: <strong>{waSessionInfo.name}</strong>
                  </div>
                  <div>
                    Protocol:{' '}
                    <strong className="mono" style={{ fontSize: 10 }}>
                      {waSessionInfo.protocol}
                    </strong>
                  </div>
                  <div>
                    Connected: <strong style={{ fontSize: 10 }}>{waSessionInfo.connectedAt}</strong>
                  </div>
                </div>
              </div>
            )}
            ...
            {waQrVisible && !waConnected && (
              ...
              >
                {waQrCode ? (
                  <div style={{ marginBottom: 12 }}>
                    {waQrCode.startsWith('data:') ? (
                      ...
                      <QRCodeCanvas text={waQrCode} size={220} />
                    )}
                  </div>
                ) : (
                  <div
                ...
                {waConnecting && waQrCode && (
                  ...
                  >
                    <RefreshCw size={14} style={{ animation: 'spin 1s linear infinite' }} /> Waiting for scan...
                  </div>
                )}
              </div>
            )}
            ...
            {!(hasPermission('whatsapp') || waAllowedSenders.includes(currentUser?.username)) && !waConnected && (
              ...
              >
                <Shield size={14} /> Only authorized users can connect WhatsApp. Contact admin to be assigned as a
                sender.
              </div>
            )}
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {!waConnected ? (
                <button
                  ...
                  disabled={
                    !(hasPermission('whatsapp') || waAllowedSenders.includes(currentUser?.username)) || waConnecting
                  }
                  style={{
                    opacity: hasPermission('whatsapp') || waAllowedSenders.includes(currentUser?.username) ? 1 : 0.5,
                    flex: 1,
                  }}
                >
                  {waConnecting ? (
                    ...
                    <>
                      <QrCode size={14} /> {hasPermission('whatsapp') ? 'Scan QR Code' : 'Admin Only'}
                    </>
                  )}
                </button>
              ) : (
                <button className="bd" onClick={handleWaDisconnect} style={{ flex: 1 }}>
                  <WifiOff size={14} /> Disconnect Session
                </button>
              )}
            </div>
          </div>
          ...
          <div className="card" style={{ padding: '18px 20px', marginTop: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ padding: 8, background: 'linear-gradient(135deg,#7C3AED,#8B5CF6)', borderRadius: 10 }}>
              ...
              <Toggle
                active={scheduledNotifs.enabled}
                onClick={() => setScheduledNotifs((prev) => ({ ...prev, enabled: !prev.enabled }))}
                color="#7C3AED"
              />
            </div>
            ...
            {scheduledNotifs.enabled && (
              <div style={{ display: 'grid', gap: 14 }}>
                ...
                <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div>
                    <label
                      style={{ display: 'block', fontSize: 11, fontWeight: 600, color: '#64748B', marginBottom: 6 }}
                    >
                      Frequency
                    </label>
                    <select
                      value={scheduledNotifs.frequency}
                      onChange={(e) => setScheduledNotifs((prev) => ({ ...prev, frequency: e.target.value }))}
                      style={{
                        width: '100%',
                        padding: '8px 10px',
                        borderRadius: 6,
                  ...
                  <div>
                    ...
                    {(scheduledNotifs.frequency === 'weekly' || scheduledNotifs.frequency === 'biweekly') && (
                      <select
                        value={scheduledNotifs.dayOfWeek}
                        onChange={(e) =>
                          setScheduledNotifs((prev) => ({ ...prev, dayOfWeek: parseInt(e.target.value) }))
                        }
                        style={{
                          width: '100%',
                          padding: '8px 10px',
                    ...
                    {scheduledNotifs.frequency === 'monthly' && (
                      <select
                        value={scheduledNotifs.dayOfMonth}
                        onChange={(e) =>
                          setScheduledNotifs((prev) => ({ ...prev, dayOfMonth: parseInt(e.target.value) }))
                        }
                        style={{
                          width: '100%',
                          padding: '8px 10px',
                    ...
                    {scheduledNotifs.frequency === 'daily' && (
                      <input
                        type="time"
                        value={scheduledNotifs.time}
                        onChange={(e) => setScheduledNotifs((prev) => ({ ...prev, time: e.target.value }))}
                        style={{
                          width: '100%',
                          padding: '8px 10px',
                          borderRadius: 6,
                ...
                {/* Delivery Channels */}
                <div>
                  <label style={{ display: 'block', fontSize: 11, fontWeight: 600, color: '#64748B', marginBottom: 8 }}>
                    Delivery Channels
                  </label>
                  <div style={{ display: 'flex', gap: 16 }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={scheduledNotifs.emailEnabled}
                        onChange={(e) => setScheduledNotifs((prev) => ({ ...prev, emailEnabled: e.target.checked }))}
                      />
                    ...
                    <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
                      <input
                        type="checkbox"
                        checked={scheduledNotifs.whatsappEnabled}
                        onChange={(e) => setScheduledNotifs((prev) => ({ ...prev, whatsappEnabled: e.target.checked }))}
                      />
                      <MessageSquare size={14} color={scheduledNotifs.whatsappEnabled ? '#25D366' : '#9CA3AF'} />{' '}
                      WhatsApp
                    </label>
                  </div>
                </div>
                    ...
                    {[
                      ...
                      <label
                        ...
                        style={{
                          ...
                          gap: 8,
                          padding: 10,
                          background: scheduledNotifs.reports?.[r.key] ? '#EDE9FE' : '#F8FAFB',
                          borderRadius: 8,
                          cursor: 'pointer',
                          border: scheduledNotifs.reports?.[r.key] ? '1px solid #C4B5FD' : '1px solid #E8ECF0',
                        }}
                      >
                        <input
                          type="checkbox"
                          checked={scheduledNotifs.reports?.[r.key]}
                          onChange={(e) =>
                            setScheduledNotifs((prev) => ({
                              ...prev,
                              reports: { ...prev.reports, [r.key]: e.target.checked },
                            }))
                          }
                          style={{ marginTop: 2 }}
                        />
                        <div>
                          <div
                            style={{
                              fontSize: 12,
                              fontWeight: 600,
                              color: scheduledNotifs.reports?.[r.key] ? '#5B21B6' : '#374151',
                            }}
                          >
                            {r.label}
                          </div>
                      ...
                      .map((u) => (
                        <label
                          ...
                          style={{
                            ...
                            gap: 4,
                            padding: '4px 10px',
                            background: scheduledNotifs.recipients?.includes(u.email) ? '#D1FAE5' : '#F8FAFB',
                            borderRadius: 16,
                            fontSize: 11,
                            cursor: 'pointer',
                            border: '1px solid #E8ECF0',
                        ...
                        >
                          <input
                            type="checkbox"
                            checked={scheduledNotifs.recipients?.includes(u.email)}
                            onChange={(e) => {
                              if (e.target.checked)
                                setScheduledNotifs((prev) => ({
                                  ...prev,
                                  recipients: [...(prev.recipients || []), u.email],
                                }));
                              else
                                setScheduledNotifs((prev) => ({
                                  ...prev,
                                  recipients: (prev.recipients || []).filter((r) => r !== u.email),
                                }));
                            }}
                            style={{ display: 'none' }}
                          />
                          <span
                            style={{ color: scheduledNotifs.recipients?.includes(u.email) ? '#059669' : '#64748B' }}
                          >
                            {u.name}
                          </span>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>
                        ...
                        {scheduledNotifs.frequency === 'daily'
                          ? 'Today'
                          : scheduledNotifs.frequency === 'weekly'
                            ? ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][
                                scheduledNotifs.dayOfWeek
                              ]
                            : scheduledNotifs.frequency === 'monthly'
                              ? 'Day ' + scheduledNotifs.dayOfMonth
                              : 'Every 2 weeks'}{' '}
                        at {scheduledNotifs.time}
                      </div>
                      <div style={{ fontSize: 11, color: '#94A3B8', marginTop: 2 }}>
                        {Object.values(scheduledNotifs.reports).filter(Boolean).length} reports •{' '}
                        {(scheduledNotifs.recipients || []).length} recipients •{' '}
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (reportCount === 0) {
                          notify('No Reports', 'Please select at least one report to include', 'warning');
                          return;
                        }
                        if (!scheduledNotifs.emailEnabled && !scheduledNotifs.whatsappEnabled) {
            ...
            {!scheduledNotifs.enabled && (
              <div style={{ fontSize: 12, color: '#9CA3AF', fontStyle: 'italic' }}>
                Enable to automatically send scheduled reports to your team via Email and WhatsApp.
              </div>
            )}
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 12 }}>
              <button
                ...
                onClick={async () => {
                  try {
                    if (api) {
                      await api.setConfigKey('scheduledNotifs', scheduledNotifs);
                      notify('Schedule Saved', 'Schedule settings saved successfully', 'success');
                    }
                  } catch (e) {
                    notify('Save Failed', 'Could not save schedule settings: ' + e.message, 'error');
                  }
                }}
              >
                <CheckCircle size={13} style={{ marginRight: 4 }} /> Save Schedule Settings
              </button>
            </div>
          </div>
          ...
          <div
            ...
            style={{
              padding: '18px 20px',
              marginTop: 16,
              border: waAutoReply ? '2px solid #0B7A3E' : '2px solid transparent',
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                <div style={{ padding: 6, background: waAutoReply ? '#D1FAE5' : '#F3F4F6', borderRadius: 8 }}>
                  <Bot size={16} color={waAutoReply ? '#059669' : '#9CA3AF'} />
                </div>
                <div>
            ...
            {waAutoReply && (
              <div style={{ background: '#F8FAFB', borderRadius: 8, padding: 12 }}>
                ...
                >
                  <Zap size={12} /> Bot uses same logic as in-app AI Assistant
                </div>
              </div>
            )}
            {!waAutoReply && (
              <div style={{ fontSize: 12, color: '#9CA3AF', fontStyle: 'italic' }}>
                Enable to let the bot automatically reply to incoming WhatsApp messages based on keywords.
              </div>
            )}
          </div>
        </div>
        ...
        <div>
          <div className="card" style={{ padding: '20px 24px', marginBottom: 16 }}>
            ...
            {!waConnected && (
              ...
              >
                <WifiOff size={13} /> Connect WhatsApp first to send messages
              </div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.value !== 'custom') setWaMessageText(waTemplates[e.target.value]?.() || '');
                  }}
                  style={{ width: '100%' }}
                >
              ...
              <div>
                ...
                <select value={waRecipient} onChange={(e) => setWaRecipient(e.target.value)} style={{ width: '100%' }}>
                  ...
                  {users
                    .filter((u) => u.status === 'active' && u.phone)
                    .map((u) => (
                      <option key={u.id} value={`${u.phone} (${u.name})`}>
                        {u.name} — {u.phone}
                      </option>
                    ))}
              ...
              <button
                ...
                style={{ opacity: waConnected ? 1 : 0.5 }}
              >
                <Send size={14} /> Send via Baileys
              </button>
            </div>
          </div>
              ...
              {waMessages.map((m) => (
                <div key={m.id} style={{ padding: '10px 12px', borderBottom: '1px solid #F0F2F5', fontSize: 12 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <Pill
                      bg={m.status === 'read' ? '#DBEAFE' : m.status === 'delivered' ? '#D1FAE5' : '#FEF3C7'}
                      color={m.status === 'read' ? '#2563EB' : m.status === 'delivered' ? '#059669' : '#D97706'}
                    >
