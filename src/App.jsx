...
const [selectedUser, setSelectedUser] = useState(null);
  ...
  const confirmArrival = useCallback((orderId) => {
    ...
    const updates = { qtyReceived: val, backOrder: val - order.quantity, status, arrivalDate: new Date().toISOString().slice(0, 10) };
  ...
  const batchConfirmArrival = useCallback((orderIds) => {
    ...
    orderIds.forEach(orderId => {
      ...
      const val = pending ? pending.qtyReceived : (order.qtyReceived || 0);
      const status = val >= order.quantity ? 'Received' : order.status;
      const upd = { qtyReceived: val, backOrder: val - order.quantity, status, arrivalDate: new Date().toISOString().slice(0, 10) };
  ...
  // ── Stats ──
  const stats = useMemo(() => {
    const t=orders.length, r=orders.filter(o=>o.status==='Received').length, b=orders.filter(o=>o.arrivalDate&&(o.qtyReceived||0)<o.quantity).length;
  ...
  const statusTrendData = useMemo(() => {
    ...
    orders.forEach(o => {
      ...
      const parts = norm.split(' ');
      const shortLabel = parts.length >= 2 ? `${parts[0].slice(0,3)} '${parts[1].slice(2)}` : norm;
      if (!monthMap[shortLabel]) monthMap[shortLabel] = { name: shortLabel, 'Pending Approval': 0, Approved: 0, Received: 0, Rejected: 0, _sortKey: norm };
...
const [emailConfig, setEmailConfig] = useState({ senderEmail: 'inventory@miltenyibiotec.com', senderName: 'Miltenyi Inventory Hub', smtpHost: '', smtpPort: 587, enabled: true, approverEmail: '', approvalEnabled: true, approvalKeywords: ['approve', 'approved', 'yes', 'confirm', 'confirmed', 'ok', 'accept', 'accepted'], approvalAutoEmail: true, approvalAutoWhatsApp: true });
  ...
  const buildApprovalHtml = ({ title, headerFields, sections, footer }) => {
    const thStyle = 'padding:10px 14px;text-align:left;font-size:12px;font-weight:600;color:#4A5568;background:#F8FAFB;border-bottom:2px solid #E2E8F0;white-space:nowrap;';
    ...
    const renderTable = (cols, rows, totals) => {
      let html = '<table cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;border:1px solid #E2E8F0;border-radius:8px;overflow:hidden;margin:16px 0;">';
      html += '<thead><tr>' + cols.map(c => `<th style="${thStyle}">${c}</th>`).join('') + '</tr></thead><tbody>';
      rows.forEach((r, i) => {
        const bg = i % 2 === 0 ? '#fff' : '#FCFCFD';
        html += `<tr style="background:${bg}">` + r.map((v, ci) => `<td style="${ci === 0 ? tdMono : ci === r.length - 1 ? tdRight : tdStyle}">${v}</td>`).join('') + '</tr>';
      });
      if (totals) {
        html += `<tr style="background:#F0FDF4;font-weight:700;">` + totals.map((v, ci) => `<td style="${ci === totals.length - 1 ? tdRight + 'color:#0B7A3E;' : tdStyle + 'font-weight:700;'}">${v}</td>`).join('') + '</tr>';
      }
      html += '</tbody></table>';
      return html;
    };
    ...
    body += `<table style="margin-bottom:20px;">` + headerFields.map(([l, v]) => `<tr><td style="padding:4px 16px 4px 0;font-size:12px;color:#64748B;font-weight:600;">${l}</td><td style="padding:4px 0;font-size:13px;font-weight:600;">${v}</td></tr>`).join('') + `</table>`;
    sections.forEach(s => {
      if (s.heading) body += `<h3 style="font-size:14px;font-weight:700;color:#1E293B;margin:24px 0 8px;border-bottom:2px solid #E2E8F0;padding-bottom:6px;">${s.heading}</h3>`;
  ...
  const trySendHtmlEmail = async (to, subject, html) => {
    ...
    try {
      const ok = await api.sendEmail({ to, subject, html, smtp: { host: emailConfig.smtpHost, port: emailConfig.smtpPort, user: emailConfig.smtpUser || '', pass: emailConfig.smtpPass || '', from: `"${emailConfig.senderName}" <${emailConfig.senderEmail}>` } });
      return ok;
    } catch { return false; }
  };
...
{page==='allorders'&&(<div>
  ...
  <div className="card" style={{padding:'20px 24px',marginTop:16}}>
    <h3 style={{fontSize:15,fontWeight:600,marginBottom:16}}>Orders by Month <span style={{fontWeight:400,fontSize:12,color:'#64748B'}}>(Click to view orders)</span></h3>
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(180px,1fr))',gap:12}}>
      {allOrdersMonths.map(month=>{
        ...
        const totalCost=orders.filter(o=>o.month===month).reduce((s,o)=>{const cp=catalogLookup[o.materialNo];const price=cp?(cp.sg||cp.tp||cp.dist||0):Number(o.listPrice)||0;return s+(price>0?price*o.quantity:(Number(o.totalCost)||0));},0);
        const isExpanded = expandedAllMonth===month;
        return <div key={month} onClick={()=>{setExpandedAllMonth(isExpanded?null:month);setExpandedAllBulkGroup(null);}} style={{padding:14,borderRadius:10,background:isExpanded?'#E6F4ED':'#F8FAFB',border:isExpanded?'2px solid #0B7A3E':'1px solid #E8ECF0',cursor:'pointer',transition:'all 0.2s'}}>
          <div style={{fontWeight:600,fontSize:12,marginBottom:8,color:'#0B7A3E',display:'flex',alignItems:'center',gap:6}}><Calendar size={12}/> {month} {isExpanded&&<ChevronDown size={12}/>}</div>
  ...
  {expandedAllMonth&&(()=>{
    ...
    return (<div style={{marginTop:16}}>
      ...
      {monthBulkGroupIds.length>0&&<div className="card" style={{padding:'20px 24px',marginBottom:16}}>
        <h3 style={{fontSize:14,fontWeight:600,marginBottom:14,display:'flex',alignItems:'center',gap:8}}><Layers size={15} color="#4338CA"/> Bulk Groups — {expandedAllMonth}</h3>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',gap:12}}>
          {monthBulkGroupIds.map(bgId=>{
            ...
            return <div key={bgId} onClick={()=>setExpandedAllBulkGroup(isExp?null:bgId)} style={{padding:14,borderRadius:10,background:isExp?'#EDE9FE':'#F8FAFB',border:isExp?'2px solid #7C3AED':'1px solid #E8ECF0',cursor:'pointer',transition:'all 0.2s'}}>
              <div style={{fontWeight:600,fontSize:12,marginBottom:4,color:'#4338CA',display:'flex',alignItems:'center',gap:6}}>{bgId} {isExp&&<ChevronDown size={12}/>}</div>
              <div style={{fontSize:11,color:'#64748B',marginBottom:4}}>By: {bg?.createdBy||'—'}</div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:4,fontSize:11}}>
                <div>Orders: <strong>{bgOrds.length}</strong></div>
                <div>Qty: <strong>{bgOrds.reduce((s,o)=>s+o.quantity,0)}</strong></div>
                <div style={{gridColumn:'span 2'}}>Cost: <strong className="mono">{fmt(bgOrds.reduce((s,o)=>{const cp=catalogLookup[o.materialNo];const price=cp?(cp.sg||cp.tp||cp.dist||0):o.listPrice;return s+(price>0?price*o.quantity:o.totalCost);},0))}</strong></div>
              </div>
              <div style={{marginTop:6}}><Badge status={bg?.status||'Pending Approval'}/></div>
            </div>;
          })}
        </div>
        ...
        {expandedAllBulkGroup&&(()=>{
          ...
          return <div style={{marginTop:16}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
              <h4 style={{fontSize:13,fontWeight:600,color:'#4338CA',display:'flex',alignItems:'center',gap:6}}><Eye size={14}/> {expandedAllBulkGroup} — {bgOrds.length} orders</h4>
              <button onClick={(e)=>{e.stopPropagation();setExpandedAllBulkGroup(null);}} style={{background:'none',border:'none',cursor:'pointer'}}><X size={16} color="#64748B"/></button>
            </div>
            <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
              <thead><tr style={{background:'#F8FAFB'}}>{['Order ID','Material No','Description','Qty','Unit Price','Total','Ordered By','Date','Status','Arrival'].map(h=><th key={h} className="th">{h}</th>)}</tr></thead>
              <tbody>{bgOrds.map((o,i)=>(
                <tr key={o.id} className="tr" onClick={()=>openOrderInNewTab(o)} style={{borderBottom:'1px solid #F7FAFC',cursor:'pointer',background:i%2===0?'#fff':'#FCFCFD'}}>
                  ...
                  <td className="td" style={{fontSize:11,maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{o.description}</td>
                  <td className="td" style={{fontWeight:600,textAlign:'center'}}>{o.quantity}</td>
                  <td className="td mono" style={{fontSize:11}}>{(()=>{const cp=catalogLookup[o.materialNo];const price=cp?(cp.sg||cp.tp||cp.dist||0):o.listPrice;return price>0?fmt(price):'—';})()}</td>
                  <td className="td mono" style={{fontSize:11,fontWeight:600}}>{(()=>{const cp=catalogLookup[o.materialNo];const price=cp?(cp.sg||cp.tp||cp.dist||0):o.listPrice;const total=price>0?price*o.quantity:o.totalCost;return total>0?fmt(total):'—';})()}</td>
                  <td className="td" style={{fontSize:11}}>{o.orderBy||'—'}</td>
            ...
            <div style={{marginTop:8,padding:10,background:'#F8FAFB',borderRadius:8,fontSize:12}}>
              <strong>Summary:</strong> {bgOrds.length} orders | Qty: {bgOrds.reduce((s,o)=>s+o.quantity,0)} | Cost: <strong className="mono">{fmt(bgOrds.reduce((s,o)=>{const cp=catalogLookup[o.materialNo];const price=cp?(cp.sg||cp.tp||cp.dist||0):o.listPrice;return s+(price>0?price*o.quantity:o.totalCost);},0))}</strong>
            </div>
          </div>;
        })()}
      </div>}
      ...
      {monthSingleOrders.length>0&&<div className="card" style={{padding:'20px 24px'}}>
        <h3 style={{fontSize:14,fontWeight:600,marginBottom:14,display:'flex',alignItems:'center',gap:8}}><Package size={15} color="#2563EB"/> Single Orders — {expandedAllMonth} <span style={{fontWeight:400,fontSize:12,color:'#64748B'}}>({monthSingleOrders.length} orders)</span></h3>
        <div style={{overflowX:'auto'}}><table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
          <thead><tr style={{background:'#F8FAFB'}}>{['Order ID','Material No','Description','Qty','Unit Price','Total','Ordered By','Date','Status','Arrival'].map(h=><th key={h} className="th">{h}</th>)}</tr></thead>
          <tbody>{monthSingleOrders.map((o,i)=>(
            <tr key={o.id} className="tr" onClick={()=>openOrderInNewTab(o)} style={{borderBottom:'1px solid #F7FAFC',cursor:'pointer',background:i%2===0?'#fff':'#FCFCFD'}}>
              <td className="td mono" style={{fontSize:11,fontWeight:600,color:'#0B7A3E'}}>{o.id}</td>
              <td className="td mono" style={{fontSize:10}}>{o.materialNo||'—'}</td>
              <td className="td" style={{fontSize:11,maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{o.description}</td>
              <td className="td" style={{fontWeight:600,textAlign:'center'}}>{o.quantity}</td>
              <td className="td mono" style={{fontSize:11}}>{(()=>{const cp=catalogLookup[o.materialNo];const price=cp?(cp.sg||cp.tp||cp.dist||0):o.listPrice;return price>0?fmt(price):'—';})()}</td>
        ...
        <div style={{marginTop:8,padding:10,background:'#F8FAFB',borderRadius:8,fontSize:12}}>
          <strong>Summary:</strong> {monthSingleOrders.length} orders | Qty: {monthSingleOrders.reduce((s,o)=>s+o.quantity,0)} | Cost: <strong className="mono">{fmt(monthSingleOrders.reduce((s,o)=>{const cp=catalogLookup[o.materialNo];const price=cp?(cp.sg||cp.tp||cp.dist||0):o.listPrice;return s+(price>0?price*o.quantity:o.totalCost);},0))}</strong>
        </div>
      </div>}
    </div>);
  })()}
...
{page==='delivery'&&(<div>
  ...
  {(()=>{
    ...
    {[
      ...
      {l:'Back Order',v:ao.filter(o=>o.arrivalDate&&(o.qtyReceived||0)<o.quantity).length,c:'#DC2626'},
      {l:'Items Pending',v:ao.reduce((s,o)=>{const diff=o.quantity-(o.qtyReceived||0);return s+(diff>0?diff:0);},0),c:'#7C3AED'}
    ].map((s,i)=><div key={i} className="card" style={{padding:'18px 22px',borderLeft:`3px solid ${s.c}`}}><div style={{fontSize:11,color:'#94A3B8',textTransform:'uppercase',letterSpacing:.5,marginBottom:4}}>{s.l}</div><div className="mono" style={{fontSize:28,fontWeight:700,color:s.c}}>{s.v}</div></div>)}
  </div>;
  })()}
                  ...
                  <tbody>
                    {bgOrders.map((o,idx)=>{
                      ...
                      <tr key={o.id} style={{borderBottom:'1px solid #F0F2F5',background:hasPending?'#FFFBEB':'transparent'}}>
                        ...
                        <td className="td" style={{textAlign:'center'}}>
                          <input type="number" min="0" max={o.quantity} value={dispQty} disabled={o.approvalStatus!=='approved'} title={o.approvalStatus!=='approved'?'Order must be approved first':''} onChange={e=>{const val=Math.max(0,Math.min(o.quantity,parseInt(e.target.value)||0));setPendingArrival(prev=>({...prev,[o.id]:{qtyReceived:val,backOrder:val-o.quantity}}));}} style={{width:50,padding:'4px 6px',textAlign:'center',borderRadius:6,border:hasPending?'2px solid #F59E0B':'1px solid #E2E8F0',fontSize:12,opacity:o.approvalStatus!=='approved'?0.5:1,cursor:o.approvalStatus!=='approved'?'not-allowed':'text'}}/>
                        </td>
                        <td className="td" style={{textAlign:'center',fontWeight:600,color:dispBO<0?'#DC2626':'#059669'}}>{dispBO<0?dispBO:'✓'}</td>
                        <td className="td"><Pill bg={(o.qtyReceived||0)>=o.quantity&&o.quantity>0?'#D1FAE5':o.arrivalDate&&(o.qtyReceived||0)<o.quantity?'#FEE2E2':'#FEF3C7'} color={(o.qtyReceived||0)>=o.quantity&&o.quantity>0?'#059669':o.arrivalDate&&(o.qtyReceived||0)<o.quantity?'#DC2626':'#D97706'}>{(o.qtyReceived||0)>=o.quantity&&o.quantity>0?`${o.qtyReceived||0}/${o.quantity} Arrived`:o.arrivalDate&&(o.qtyReceived||0)<o.quantity?`${o.qtyReceived||0}/${o.quantity} Back Order`:`0/${o.quantity} Awaiting`}</Pill></td>
                        <td className="td">
                          <button className={hasPending||!o.arrivalDate?"bp":"bs"} disabled={!hasPending&&!!o.arrivalDate} onClick={()=>confirmArrival(o.id)} style={{padding:'4px 10px',fontSize:11,borderRadius:6,opacity:hasPending||!o.arrivalDate?1:0.4,cursor:hasPending||!o.arrivalDate?'pointer':'default'}}>{hasPending?(o.arrivalDate?'Update':'Confirm'):o.arrivalDate?(o.status==='Received'?'✓ Done':'Confirmed'):'Confirm'}</button>
                        </td>
                      </tr>);
                    })}
                  </tbody>
          ...
          <tbody>
            {indivOrders.map(o=>{
              ...
              <tr key={o.id} style={{borderBottom:'1px solid #F0F2F5',background:hasPending?'#FFFBEB':'transparent'}}>
                ...
                <td className="td" style={{textAlign:'center',fontWeight:600}}>{o.quantity}</td>
                <td className="td" style={{textAlign:'center'}}>
                  <input type="number" min="0" max={o.quantity} value={dispQty} disabled={o.approvalStatus!=='approved'} onChange={e=>{const val=Math.max(0,Math.min(o.quantity,parseInt(e.target.value)||0));setPendingArrival(prev=>({...prev,[o.id]:{qtyReceived:val,backOrder:val-o.quantity}}));}} style={{width:50,padding:'4px 6px',textAlign:'center',borderRadius:6,border:hasPending?'2px solid #F59E0B':'1px solid #E2E8F0',fontSize:12}}/>
                </td>
                <td className="td" style={{textAlign:'center',fontWeight:600,color:dispBO<0?'#DC2626':'#059669'}}>{dispBO<0?dispBO:'\u2713'}</td>
                <td className="td"><Pill bg={(o.qtyReceived||0)>=o.quantity&&o.quantity>0?'#D1FAE5':o.arrivalDate&&(o.qtyReceived||0)<o.quantity?'#FEE2E2':'#FEF3C7'} color={(o.qtyReceived||0)>=o.quantity&&o.quantity>0?'#059669':o.arrivalDate&&(o.qtyReceived||0)<o.quantity?'#DC2626':'#D97706'}>{(o.qtyReceived||0)>=o.quantity&&o.quantity>0?`${o.qtyReceived||0}/${o.quantity} Arrived`:o.arrivalDate&&(o.qtyReceived||0)<o.quantity?`${o.qtyReceived||0}/${o.quantity} Back Order`:`0/${o.quantity} Awaiting`}</Pill></td>
                <td className="td">
  ...
  {hasPermission('settings') && <div className="card" style={{padding:'24px 28px',marginBottom:16}}>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      ...
      <div className="grid-2" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginTop:8}}>
        <div>
          <label style={{display:'block',fontSize:12,fontWeight:600,color:'#4A5568',marginBottom:6}}>SMTP Username</label>
          <input type="text" value={emailConfig.smtpUser||''} onChange={e=>setEmailConfig(prev=>({...prev,smtpUser:e.target.value}))} placeholder="your-email@gmail.com" style={{width:'100%'}}/>
        </div>
        <div>
          <label style={{display:'block',fontSize:12,fontWeight:600,color:'#4A5568',marginBottom:6}}>SMTP Password</label>
          <input type="password" value={emailConfig.smtpPass||''} onChange={e=>setEmailConfig(prev=>({...prev,smtpPass:e.target.value}))} placeholder="App password" style={{width:'100%'}}/>
        </div>
      </div>
      <div style={{fontSize:11,color:'#94A3B8',marginTop:6}}>Configure SMTP to send HTML emails with styled tables. Without SMTP, approval emails open in your default mail client (plain text).</div>
    </div>
  </div>}
          ...
          <div className="grid-3" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,alignItems:'end'}}>
            ...
            <div>
              ...
              <input type="number" min="0" max={selectedOrder.quantity||0} value={selectedOrder.qtyReceived||0} onChange={e=>{
                ...
                const updatedOrder = {...selectedOrder, qtyReceived: val, backOrder: newBackOrder, status: newStatus, arrivalDate: selectedOrder.arrivalDate || new Date().toISOString().slice(0,10)};
