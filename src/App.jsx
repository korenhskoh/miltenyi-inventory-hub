...
const [selectedUser, setSelectedUser] = useState(null);
  ...
  const [partsCatalog, setPartsCatalog] = useState(PARTS_CATALOG);
...
{page==='settings'&&(<div style={{maxWidth:700}}>
  ...
  {isAdmin && <div className="card" style={{padding:'24px 28px',marginBottom:16}}>
    ...
    <div style={{marginTop:12,display:'flex',gap:8}}>
      <button className="bp" style={{fontSize:12}} onClick={()=>{notify('Templates Saved','Email templates updated successfully','success');}}>Save Templates</button>
  ...
  {isAdmin && <div style={{background:'#fff',borderRadius:12,padding:20,boxShadow:'0 1px 3px rgba(0,0,0,.06)',marginBottom:20}}>
    ...
    <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
      ...
      <button onClick={()=>{if(window.confirm(`Clear all ${partsCatalog.length} parts from catalog? The default catalog will be restored on next reload.`)){setPartsCatalog([]);api.clearCatalog();notify('Catalog Cleared','Parts catalog has been cleared','info');}}} style={{padding:'8px 16px',background:'#DC2626',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><Trash2 size={14}/> Clear Catalog</button>
      <button onClick={()=>{setPartsCatalog(PARTS_CATALOG);api.uploadCatalog(PARTS_CATALOG.map(p=>({materialNo:p.m,description:p.d,category:p.c||'',sgPrice:p.sg||0,distPrice:p.dist||0,transferPrice:p.tp||0,rspEur:p.rsp||0})));notify('Catalog Restored',`Default catalog restored (${PARTS_CATALOG.length} parts)`,'success');}} style={{padding:'8px 16px',background:'#059669',color:'#fff',border:'none',borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><RefreshCw size={14}/> Restore Default</button>
    </div>
  </div>}
