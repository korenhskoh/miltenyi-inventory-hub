...
export default function App() {
  ...
  const sendScheduledReport = useCallback(
    async () => {
      ...
      if (rpts.backOrderReport) {
        ...
        const lines = backOrders
          .slice(0, 10)
          .map((o) => `• ${(o.description || '').slice(0, 35)}: ${o.qtyReceived || 0}/${o.quantity} (B/O: ${o.quantity - (o.qtyReceived || 0)})`)
          .join('\n');
        sections.push(
      ...
      if (rpts.orderStats) {
        const thisMonth = orders.filter(
          (o) => o.orderDate && o.orderDate.slice(0, 7) === now.slice(0, 7),
        ).length;
        const avgLead = (() => {
          const delivered = orders.filter((o) => o.orderDate && o.arrivalDate && o.status === 'Received');
          if (!delivered.length) return 'N/A';
          const avg = delivered.reduce((s, o) => s + (new Date(o.arrivalDate) - new Date(o.orderDate)) / 86400000, 0) / delivered.length;
  ...
  }, [scheduledNotifs.enabled, scheduledNotifs.frequency, scheduledNotifs.dayOfWeek, scheduledNotifs.dayOfMonth, scheduledNotifs.time, scheduledNotifs.lastRun, sendScheduledReport]);
